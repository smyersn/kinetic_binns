...
from modules.utils.normalization import normalize, denormalize
...
class BINN(nn.Module):
    ...
    def __init__(self, dimensions, species, input_extrema=None, mu=None, sigma=None, 
                 duplicates=1, diff_coeffs=None, degree=2, l05_reg=0, l1_reg=0, 
                 param_bounds=10):
        ...
        # Register mu and sigma as buffers
        ...
        # input extrema
        if input_extrema:
            self.x_min = min(input_extrema[0][:-1]).item()
            self.x_max = max(input_extrema[1][:-1]).item()
            self.t_min = input_extrema[0][-1].item()
            self.t_max = input_extrema[1][-1].item()
        else:
            self.x_min = 0
            self.x_max = 10
            self.t_min = 0
    ...
    def gls_loss(self, pred, true):
        ...
        residual_gamma = residual * pred.abs().clamp(min=1.0)**(-self.gamma)
        # print('GLS loss (Pred, true, residual, residual_gamma)')
        # print(torch.concat([pred[:5], true[:5], residual[:5], residual_gamma[:5]], dim=1))
        return torch.mean(residual_gamma)
    ...
    def pde_loss(self, inputs, outputs):
        ...
        # print(f'xtuv: {torch.concat([inputs, u], dim=1)}')
        # print(f'LHSU RHSU: {torch.concat([LHS_u, RHS_u], dim=1)}')
        # print(f'LHSV RHSV: {torch.concat([LHS_v, RHS_v], dim=1)}')
        # print(f'RHSU, LAPU, F: {torch.concat([RHS_u, lap_u, F], dim=1)}')
        # print(f'RHSV, LAPV, -F: {torch.concat([RHS_v, lap_v, -F], dim=1)}')
        # print(f'loss: {pde_loss}')
    ...
    def reg_loss(self):
        ...
        if self.l05_reg:
            ...
            self.sparsity_loss += self.l05_reg * l05_norm
            
        if self.l1_reg:
            l1_norm = torch.norm(coeffs, p=1)
            self.sparsity_loss += self.l1_reg * l1_norm        
    ...
    def loss(self, pred, true, epoch):
        ...
        if epoch >= 2000:
            ...
            # normalize, predict uv at sampled locations, denormalize
            inputs_rand_norm = normalize(inputs_rand, self.mu[:self.dimensions+1], 
                                        self.sigma[:self.dimensions+1])

            outputs_rand_norm = self.surface_fitter(inputs_rand_norm)
            
            outputs_rand = denormalize(outputs_rand_norm, 
                                    self.mu[self.dimensions+1:], 
                                    self.sigma[self.dimensions+1:])  
                                
            # compute PDE loss at sampled locations
            self.pde_loss_val = self.pde_weight*self.pde_loss(inputs_rand, 
                                                            outputs_rand)
            
            # compute loss from regularization
            self.reg_loss_val = self.reg_weight*self.reg_loss()
    ...
    def remove_insignificant_terms(self, uv):
        ...
        poly_features = self.reaction.eql_layer.poly(uv)
        hill_features = self.reaction.eql_layer.hill(uv)
        features_unweighted = torch.concatenate((poly_features, hill_features), dim=1)

        weights = self.reaction.eql_layer.fc.weight[0]
        features = features_unweighted * weights    
                
        sd_mask = features.std(dim=0) < 0.1

        with torch.no_grad():
            self.reaction.eql_layer.fc.weight[0][sd_mask] = 0
    ...
    def generate_equation(self):
        ...
        for term, coeff, k, n in zip(hill_terms*self.duplicates, hill_coeffs_inc, Ks_inc, ns_inc):
            if coeff != 0:
                string = f'{coeff:.5f}'
                if len(term) == 1:
                    string += f' * {species[term[0]]}^{n:.5f} / (1 + {k:.5f} * {species[term[0]]}^{n:.5f})'
                else:     
                    string += f' * {species[term[1]]} * {species[term[0]]}^{n:.5f} / (1 + {k:.5f} * {species[term[0]]}^{n:.5f})'
                terms.append(string)
        ...
        for term, coeff, k, n in zip(hill_terms*self.duplicates, hill_coeffs_dec, Ks_dec, ns_dec):
            if coeff != 0:
                string = f'{coeff:.5f}'
                if len(term) == 1:
                    string += f' * (1 - {species[term[0]]}^{n:.5f} / (1 + {k:.5f} * {species[term[0]]}^{n:.5f}))'
                else:     
                    string += f' * {species[term[1]]} * (1 - {species[term[0]]}^{n:.5f} / (1 + {k:.5f} * {species[term[0]]}^{n:.5f}))'
                terms.append(string)
