...
from modules.binn_eql_hypernet.visualize_surface import visualize_surface
from modules.binn_eql_hypernet.simulate_surface import simulate_surface, animate_sim
...
k_init=float(config['k_init'])
lambda_init=float(config['lambda_init'])
epoch_interval=int(config['epoch_interval'])
param_bounds = float(config['param_bounds'])

# Set training hyperparameters
# epochs = int(1e6)
epochs = epoch_interval * 4
# epochs = 300
...
F_mlp_unformatted, _ = model.model.reaction.eql_layer(torch.tensor(uv_nans).float().to(device), inference=True)
